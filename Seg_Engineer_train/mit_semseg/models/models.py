...
class ModelBuilder:
    ...
    @staticmethod
    def build_net(encoder_weights='', decoder_weights='', fc_dim=512, num_class=150, use_softmax=False):
        pretrained = True if len(encoder_weights) == 0 else False
        orig_resnet = resnet.__dict__['resnet101'](pretrained=pretrained)
        net_encoder = Resnet(orig_resnet)
        if len(encoder_weights) > 0:
            print('Loading weights for net_encoder')
            net_encoder.load_state_dict(
                torch.load(encoder_weights, map_location=lambda storage, loc: storage), strict=False)
